...
$.game('triple', function(container, overviewData) {
    ...
    $(`.game-container [data-triple-id]`).on('click', function () {
        ...
        $('.game-container [data-triple-id].selected > img').remove();
        $('.game-container [data-triple-id].selected').removeClass('selected');

    });

    if($.isOverview(overviewData)) {
        for(let i = 0; i < overviewData.game.data.user_tiles.length; i++)
            $(`.overview-render-target [data-triple-id="${overviewData.game.data.user_tiles[i]}"]`).addClass('active');
        $.chain(4, 100, function(i) {
		const id = i - 1;
		const user_tiles = overviewData.game.data.user_tiles[id];
		...
		if(i === 4) {
			for (let s = 0; s < 36; s++) {
			const user_tiles = overviewData.game.data.user_tiles[s];
			const tripleres = overviewData.game.data.tiles[s];
			if (s != user_tiles) {
    ...
    $.chain(4, 200, function(i) {
        ...
        if(i === 4) {
			$("[data-m=\"".concat(response.game.multiplier.toFixed(2), "\"]")).addClass('highlight');
			setGrid(response);
        }
...
$.on('/game/triple', function() {
    ...
    $.sidebar(function(component) {
        ...
        component.buttons(null, 'mt-2')
            .add($.lang('general.autopick'), function() {
                ...
                while(picked.length <= 3) {
                    let rand = $.random(0, 36);
                    if(picked.includes(rand)) continue;
                    picked.push(rand);
                }

                $.chain(3, 100, function(index) {
                    $(`[data-triple-id="${picked[index]}"]`).click();
                });
            }, 'autopick', '', false)
            .add($.lang('general.clear'), function() {
                ...
                tiles = [];
                $(`.game-container [data-triple-id]`).removeClass('active');
            }, 'clear');
        ...
        component.history('triple', true);

        const append = function(e, icon) {
            e.append(`<img class="block-icon" src="${icon}" alt="">`);
        };
	...
	$.history().add(function (e) {
      e.attr('data-m', '0.40').append("<div>x0.40</div>");
      var c = $(`<div class="block-icons"></div>`);
      e.append(c);
      append(c, purple);
      append(c, blue);
