...
$.game('slide', function (container, overviewData) {
  if (!$.isOverview(overviewData)) {
    ...
    _.forEach($.gameData().history, function (m) {
      ...
      $('.slideCustomHistory').append($.customHistoryPopover($("<div class=\"slideCustomHistoryElement\" style=\"background: ".concat(color[0], "; border-bottom: 1px solid ").concat(color[1], "\">").concat(m.multiplier.toFixed(2) + 'x', "</div>")), {
