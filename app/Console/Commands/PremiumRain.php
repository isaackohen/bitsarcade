...
/**
    ...
    public function handle() {
        ...
        $last30minute = \Carbon\Carbon::now()->subMinutes(30)->toDateTimeString();
        $last4minute = \Carbon\Carbon::now()->subMinutes(4)->toDateTimeString();
        
        $all = User::where('latest_activity', '>=', \Carbon\Carbon::parse($last30minute))->where('viplevel', '>', 1)->get()->toArray();
        if(count($all) < $usersLength) {
